...
fn update_units_file() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if std::path::PathBuf::from(UNITS_OUTPUT_FILE)
        .try_exists()
        .unwrap_or(false)
        && src_file_modified == output_file_modified
    {
        return Ok(());
    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // TODO: Enable this again with another exchange rates API!
        ...
        fn generate_lines(&self) -> Vec<String> {
            ...
            let format = |expr: &str, flip: bool| {
                let mut result = format!(
                    "(r#\"{}\"#, r#\"{}\"#) ",
                    if !flip { src } else { dst },
                    if !flip { dst } else { src }
                );

                let (src_pow, dst_pow) = if !flip {
                    (src_pow, dst_pow)
                } else {
                    (dst_pow, src_pow)
                };
            ...
            for modifier in &self.modifiers {
                if *modifier == MODIFIER_NORMAL {
                    if self.dst_to_src_expr.is_some() {
                        continue;
                    }
                    ...
                    let new_operator =
                        invert_operator(parts.next().unwrap().chars().next().unwrap());
    ...
    macro_rules! next_or_err {
        ($iter:expr, $curr_idx:expr) => {
            if let Some(v) = $iter.next() {
                ...
                return Err(Error($curr_idx).into());
            }
        };
    }
    ...
    impl Generator {
        ...
        pub fn generate(&mut self) -> std::result::Result<(), Box<dyn std::error::Error>> {
            ...
            while self.index != self.lines {
                ...
                let (src, dst) = self.parse_conversion_head(parts.next().unwrap_or_default())?;

                let expressions = next_or_err!(parts, self.index).trim().to_string();
                if expressions.is_empty() {
                    return Err(Error(self.index).into());
                }
                ...
                if let Some(semicolon_idx) = expressions.find(';') {
                    ...
                    if std_expr.is_empty() || _dts_expr.is_empty() {
                        return Err(Error(self.index).into());
                    }
                ...
                if dts_expr.is_some() && !modifiers.is_empty() {
                    return Err(Error(self.index).into());
                }

                self.conversions
                    .push(Conversion::new(src, dst, std_expr, dts_expr, modifiers));
                self.index += 1;
            }
            ...
            let unit_strings = self
                ...
                .flat_map(|u| {
                    ...
                    if self
                        .conversions
                        .iter()
                        .find(|conv| conv.src.0 == u.abbreviation || conv.dst.0 == u.abbreviation)
        ...
        fn parse_units(&mut self) -> Result<()> {
            while self.index != self.lines {
                ...
                for unit in line_parts {
                    ...
                    let Some(ci) = unit.find(':') else {
                        return Err(self.index.into());
                    };
